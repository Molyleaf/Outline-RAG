...
(function initAutoResize() {
    ...
    qEl.addEventListener('input', () => {
        if (currentEditMessageId) {
            // 如果用户在编辑时清空了输入框，则取消编辑状态
            if (qEl.value.trim() === '') {
                currentEditMessageId = null;
                document.querySelector('.input-inner')?.classList.remove('editing');
            }
        }
    });
})();
...
qEl.addEventListener('keydown', (e) => {
    // (新) 如果在编辑时按 Esc，取消编辑
    if (e.key === 'Escape' && currentEditMessageId) {
        e.preventDefault();
        currentEditMessageId = null;
        document.querySelector('.input-inner')?.classList.remove('editing');
        qEl.value = '';
...
(async function init() {
    // (新) 顶级函数，用于设置顶部栏UI，依赖 MODELS
    window.setupTopbarActions = function() {
        const actionsContainer = document.querySelector('.topbar .actions');
        if (!actionsContainer) return;

        // 清空旧按钮（以防重载）
        actionsContainer.querySelectorAll('.btn.tonal, .toolbar-popover').forEach(el => el.remove());
        ...
        function updateModelButtonLook(modelId, btnElement) {
            ...
            const iconSrc = modelConf.icon || getAvatarUrlForModel(modelId); // 回退

            if (modelId && (modelId.includes('moonshotai') || modelId.includes('Kimi'))) {
        ...
        if (uploadLabel) {
            ...
            // actionsContainer.insertBefore(tempBtn, uploadLabel); // 隐藏
            // actionsContainer.insertBefore(topPBtn, uploadLabel); // 隐藏
        }
        ...
        const mobileModelMenuHtml = () =>
            ...
            (modelEntries.length > 0 ? modelEntries.map(([id, m]) =>
                '<div class="model-item ' + (id === currentModelId ? 'active' : '') + '" data-id="' + id + '">' +
                '<img src="' + m.icon + '" alt="' + m.name + '"><span>' + m.name + (m.beta ? ' (Beta)' : '') + '</span>' +
                '</div>'
            ).join('') : '<div>没有可用的模型</div>') +
            '</div>' +
            '</div>' +
            '<div class="mobile-sheet-group">' +
        ...
        // (新) setupTopbarActions() 已被移至 app.js 的 loadUser() 末尾

        (async () => {
            try { await loadUser(); } catch(e) {
                console.error("Failed to load user, trying to init UI anyway", e);
                // 确保即使 loadUser 失败（例如 /api/me 401），UI 也能尝试初始化
                if (window.setupTopbarActions) window.setupTopbarActions();
            }
            // loadUser() 现在会处理 setupTopbarActions
            await loadConvs();
            const greet = document.getElementById('greeting');
