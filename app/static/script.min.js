...
/**
 * (新增) 辅助函数：将原始文本块作为 fade-in 动画 <span> 附加到容器
 */
...
async function sendQuestion() {
    ...
    // 4. (删除) `rerender` 函数已被删除

    // 5. (修改) fetch 和 SSE 处理循环
    ...
    try {
                ...
                if (chunk.startsWith('data:')) {
                    ...
                    try {
                        ...
                        if (typeof delta === 'string' && delta.length > 0) {
                            ...
                            const match = delta.match(triggerRegex);

                            if (match && parseFn) {
                                // 触发器命中！
                                const textBeforeTrigger = delta.substring(0, match.index);
                                const triggerAndRest = delta.substring(match.index);
                                ...
                                if (currentStreamingBuffer.trim() !== '') {
                                    ...
                                    if (window.hljs) currentStreamingDiv.querySelectorAll('pre code').forEach(block => window.hljs.highlightElement(block));
                                } else {
        ...
        finalizeStream(); // (修改) 处理流在 [DONE] 之前结束的情况
    } catch (e) {
        console.error("Stream processing error:", e);
        finalizeStream(); // (修改) 异常时也尝试最终化
        toast('连接中断', 'warning');
    }
}
// --- (修改) 结束 ---
